...
export default {
    ...
    watch: {
        // 当 visible 属性改变后并且值为 true （弹出窗口）时，去调用接口查询所有菜单，已经存在这个接口。
        visible(val) {
            if (val) {
    ...
    methods: {
        ...
        // 通过角色id查询角色之前所拥有的菜单ids ,然后进行回显
        async getMenuIdsByRoleId() {
            this.$api.getMenuIdsByRoleId(this.roleId).then(res => {
        ...
        submitForm(formName) {
            ...
            // 调用保存角色权限菜单接口
            this.$api.saveRoleMenu(this.roleId, checkedMenuIds).then(res => {
