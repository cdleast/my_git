...
export default {
    ...
    methods: {
        // 分页条件查询列表
        async fetchData() {
            await this.$api.getCategorySearchList(this.query, this.page.current, this.page.size).then(res => {
        ...
        // 编辑列表
        async handleEdit(id) {
            await this.$api.getCategoryId(id).then(res => {
        ...
        handleDelete(id) {
            this.$confirm('确认删除这条记录吗?', '提示', {
                ...
                // 发送删除请求
                this.$api.deleteCategoryId(id).then(res => {
                    this.$message({
