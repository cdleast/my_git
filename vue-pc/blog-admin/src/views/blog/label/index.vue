<template>
    <div class="app-container">
        ...
        <!-- 新增和编辑弹窗 -->
        <addEdit :title="addEdit.title" :visible="addEdit.visible" :formData="addEdit.formData" :normalList="normalList" :remoteClose="remoteClose"></addEdit>
    </div>
</template>
...
export default {
    ...
    methods: {
        // 分页条件查询列表
        async fetchData() {
            await this.$api.getLabelList(this.query, this.page.current, this.page.size).then(res => {
        ...
        // 编辑
        async handleEdit(id) {
            await this.$api.getLabel(id).then(res => {
                if (res.code === 20000) {
        ...
        handleDelete(id) {
            this.$confirm('确认删除这条记录吗?', '提示', {
                ...
                // 发送删除请求
                this.$api.deleteLabel(id).then(res => {
                    // 处理响应结果提示
                    this.$message({
